...
def get_dataset(dst_name, model_name, seed=None):
    if dst_name == 'wheat_corn_reuters':
        ...
        pool_set = load_dataset('csv', data_files=DATA_POOL_PATH+'pool.csv')['train']
        dst = post_process(pool_set, model_name)
        dst.set_format('torch')
        return dst
    elif dst_name == 'augmented_wheat_corn_reuters_valset':
        import numpy as np
        dst_indexes = np.loadtxt(DATA_POOL_PATH+'reuters_wheat_corn_valset_100p' + str(seed) + '.txt')
        dst_indexes = dst_indexes.astype(dtype=int)
        augment_dst = load_dataset('csv', data_files=DATA_POOL_PATH+'pool.csv')['train']
        ...
        import numpy as np
        dst_indexes = np.loadtxt(DATA_POOL_PATH+'byorder/reuters_wheat_corn_valset_100p' + str(seed) + '.txt')
        dst_indexes = dst_indexes.astype(dtype=int)
        augment_dst = load_dataset('csv', data_files=DATA_POOL_PATH+'pool.csv')['train']
        dst = augment_dst.select(dst_indexes.tolist())
        del augment_dst
